...
class System
{
	...
	public function __construct()
	{
		...
		if(!$ajax_request && !isset($_SESSION['mv']['user']['id'], $_SESSION['mv']['user']['password']))
			if(isset($_COOKIE["remember_key"], $_COOKIE["remember_code"])) //If auto login
			{
				$login = new Login();
								
				if($id = $login -> autoLogin($_COOKIE["remember_key"], $_COOKIE["remember_code"]))
				{
		...
		{
			if(isset($_COOKIE["remember_key"], $_COOKIE["remember_code"]))
			{
				$login = new Login();
				$autologin_id = $login -> autoLogin($_COOKIE["remember_key"], $_COOKIE["remember_code"]);
	...
	public function ajaxRequestCheck()
	{
		...
		if(!is_object($this -> user) || !$this -> user -> checkUserLogin())
		{
			...
			{
				$login = new Login();
				$autologin = $login -> autoLogin($_COOKIE["remember_key"], $_COOKIE["remember_code"]);
